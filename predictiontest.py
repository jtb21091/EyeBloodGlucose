...
def get_pupil_size(image):
    ...
    try:
        ...
        circles = cv2.HoughCircles(
            gray_blurred,
            cv2.HOUGH_GRADIENT,
            dp=1.0,         # Slightly adjusted from 1.2 for a tighter accumulator resolution.
        ...
        if circles is not None:
            ...
            # Return the average radius from the detected circles.
...
def get_sclera_redness(image):
    ...
    try:
        ...
        # Define two masks for red hues (lower and upper ranges).
...
class EyeGlucoseMonitor:
    ...
    def compute_pupil_response_time(self) -> float:
        ...
        Defined as the inverse of the absolute dilation rate.
        """
    ...
    def extract_features(self, frame: np.ndarray) -> Dict:
        ...
        # New dynamic feature (for analysis): variance of pupil sizes over the recent history.
        if len(self.pupil_history) < 2:
            pupil_variance = 0.0
        else:
        ...
        features = {
            ...
            "pupil_variance": pupil_variance  # extra, for internal analysis (not sent to the model)
        }
        # Enforce the order expected by the model (omit extra keys if necessary).
    ...
    def predict_glucose(self, features: Dict):
        ...
        for smoothing.
        Only passes the features expected by the model.
        Also computes a confidence level based on prediction uncertainty.
        """
        result = None
        confidence = None
        try:
            if self.model is not None and features:
                # If the model stores its training feature names, filter out any extras.
                ...
                # If the model is an ensemble (e.g., RandomForestRegressor), compute the standard deviation across trees.
                if hasattr(self.model, "estimators_"):
                    try:
                        all_preds = np.array([estimator.predict(df)[0] for estimator in self.model.estimators_])
                        std_prediction = np.std(all_preds)
                        # Here we compute a confidence level between 0 and 1; lower standard deviation gives higher confidence.
                        confidence = 1.0 / (1.0 + std_prediction)
                    except Exception as e:
                        logging.error("Error computing ensemble confidence: " + str(e))
                        confidence = None
                # If using a GaussianProcessRegressor (or similar) that supports return_std.
                    ...
                    confidence = 1.0  # Default confidence if uncertainty cannot be computed.
        except Exception as e:
    ...
    def run(self):
        ...
        while True:
            ...
            # Display prediction and confidence text on the frame.
            with self.prediction_lock:
